...
BALL_Draw:
        FDB     BALL_0_0            ; Address to draw semigraphics sprite
        FDB     BALL_0_1            ; Address to draw semigraphics sprite
...
; Row 0 
        ...
        LDA     8,X                 ; read the sprite data on screen 0
        STA     8,Y                 ; write the sprite data on screen 1
        ABX                         ; Move down a row
...
Restore_BALL_0:
        ...
        LDS     #BALL_BackupStart+32    ; Set S pointer to the end of the last row of the backup buffer + 32 extra space for stack during F/IRQ
        LEAU    1001,X              ; Point at the bottom, right edge of the sprite backup location
; Restoring row 31 
        ...
        LEAU    -23,U               ; Move to the correct position to write data on screen
; Restoring row 30 
        PULS    D                   ; Get Data to restore
        PSHU    D                   ; Write a Data to the screen
        PULS    D,DP,X,Y            ; Get Data to restore
...
; Backup Sprite data for BALL
; Enter with X pointing at the memory location on screen to backup the data behind the sprite
; Sprite Width is: 9 Bytes to backup
; Height is: 32 Rows
BALL_BackupStart:
        RMB     9*32+32             ; Reserve space for sprite background, plus a little extra for the stack (If F/IRQ happens)
BALL_BackupEnd:
Backup_BALL:
        PSHS    DP                  ; Save Condition Codes & DP
        STS     @SaveSHere+2        ; Backup the stack pointer's value at the end of the backup routine (self mod)
        LDS     #BALL_BackupEnd     ; Set S pointer to the end of the backup buffer
        LEAU    ,X                  ; U = X
; Backup row 0 
        ...
        LEAU    32-9,U              ; Move down to the start of the next row to copy
; Backup row 1 
        PULU    D,DP,X,Y            ; Get seven bytes from the screen
        PSHS    D,DP,X,Y            ; Save seven bytes from the screen
        PULU    D                   ; Get two bytes from the screen
