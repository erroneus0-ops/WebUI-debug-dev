...
; Numeric Variables Used: 6 
_Var_BY    RMB      2     Type of variable is: Integer
_Var_VY    RMB      2     Type of variable is: Integer
_Var_DOMECUR    RMB      2     Type of variable is: Integer
_Var_DOMENEXT    RMB      2     Type of variable is: Integer
_Var_COLORIDX    RMB      2     Type of variable is: Integer
_Var_FRAMECOUNT    RMB      2     Type of variable is: Integer
...
_StrVar_A                              
        RMB     1             ; String Variable A length (0 to 255) initialized to 0
        RMB     255           ; 255 bytes available for string variable A
; Numeric Arrays Used: 0 
; String Arrays Used: 0 
EndClearHere2nd:
; General Commands Used: 23 
; '
; DIM
; AS
; INTEGER
...
Artifacting      EQU     0         ; Not using Artifact colours                              
        INCLUDE     ./SAUCER_ONE.asm
SpriteDrawTable:                              
        FDB     SAUCER_ONE_Draw   ; Points to the Sprite Drawing Table (in the compiled sprite.asm file)
        ...
        FDB     $0000         ; No sprite loaded in this slot
SpriteBackupTable:                              
        FDB     Backup_SAUCER_ONE   ; Address of the Make Backup code
        ...
        FDB     $0000         ; No Sprite for this slot
SpriteRestoreTable:                              
        FDB     Restore_SAUCER_ONE_0   ; Address of the restore code Buffer 0
        FDB     Restore_SAUCER_ONE_1   ; Address of the restore code Buffer 1
...
; '  size ceiling entirely (confirmed:everydomecellcurrentlyholds 
...
; BY =0
        LDD     #$0000        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_BY       ; Save Numeric variable
; VY =1
...
; DOMECUR =191
        LDD     #$00BF        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_DOMECUR  ; Save Numeric variable
; COLORIDX =0
        LDD     #$0000        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_COLORIDX ; Save Numeric variable
; FRAMECOUNT =0
        LDD     #$0000        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_FRAMECOUNT   ; Save Numeric variable
_LMainLoop
...
;        PULS    B             ; Get value in B and Fix the stack
        ...
        LDB     #10           ; B is an 8 bit integer from 0 to 255 (_Unsigned _Byte) ~%% format
        PSHS    B             ; Save B on the stack
        CLRA                  ; Clear MSB of D
        PULS    B             ; Get value in B and Fix the stack
        PSHS    D             ; Save the x co-ordinate
        LDD     _Var_BY       ; Get the value of the variable
...
; A$=INKEY$
; Handling String command here                      
        JSR     StrCommandInkey   ; If a key is pressed the string is on the stack, otherwise a zero is on the stack
; String is on the stack, deal with it here:                      
        PULS    B             ; B = length of the source string
        LDX     #_StrVar_A    ; X points at the length of the destination string
        STB     ,X+           ; Set the size of the destination string, X now points at the beginning of the destination data
        BEQ     @Done         ; If the length of the string is zero then don't copy it (Skip ahead)
!       LDA     ,S+           ; Get a source byte
...
; IF A$<>""THEN 
; Starting if                      
; Copy string to the stack:                      
        LDX     #_StrVar_A    ; X points at source string
        LDB     ,X+           ; B = length of the source string
        BNE     >             ; Loop
        CLR     ,-S           ; Save a zero on the stack
        BRA     @Done         ; Skip ahead
!       ABX                   ; Move X to the end of the string
...
@Done:                        
        ...
        BSR     @CopyToStack  ; Skip ahead
        FCB     $00           ; Length of the string
@CopyToStack                      
        LDB     #$01          ; Length of the string
        PULS    U             
!       LDA     ,U+           
        STA     ,-S           
        DECB                  
...
; Spill top string from stack into _StrVar_PF01:                      
        PULS    B             ; B = length of the source string
        LDX     #_StrVar_PF01 ; X points at destination string
        STB     ,X+           ; Store destination length
        BEQ     @Done         ; If zero length then done
!       LDA     ,S+           ; Get source byte
        STA     ,X+           ; Store destination byte
        DECB                  ; Decrement count
        BNE     <             ; Loop until copied
@Done                         

; Spill top string from stack into _StrVar_PF00:                      
        PULS    B             ; B = length of the source string
        LDX     #_StrVar_PF00 ; X points at destination string
        STB     ,X+           ; Store destination length
        BEQ     @Done         ; If zero length then done
!       LDA     ,S+           ; Get source byte
        STA     ,X+           ; Store destination byte
...
; Copy named string to the stack:                      
        LDX     #_StrVar_PF01 ; X points at source string
        LDB     ,X+           ; B = length of the source string
        BNE     >             ; Loop
        CLR     ,-S           ; Save a zero on the stack
        BRA     @Done         ; Skip ahead
...
; Copy named string to the stack:                      
        LDX     #_StrVar_PF00 ; X points at source string
        LDB     ,X+           ; B = length of the source string
        BNE     >             ; Loop
        CLR     ,-S           ; Save a zero on the stack
        BRA     @Done         ; Skip ahead
...
@Done:                        
        ...
        JSR     StrCompare    ; Compare strings on stack, A = -1/0/+1, B=$FF
        BNE     >             ; If non-zero then not equal
        CLRB                  ; Flag as false
!                             
        PSHS    B             ; Push True/False (byte) on stack
; Expression is parsed                      
        LDB     ,S+           ; Get final boolean result
        LBEQ    _IFDone_01    ; If FALSE, go to ELSE/END IF
_IFTrue_01                      
; END 
        JMP     EXITProgram   ; All done, Exit the program
; END IF 
...
; Starting if                      
        LDD     _Var_BY       ; Get the value of the variable
        PSHS    D             ; Save the value on the stack
        LDB     #67           ; B is an 8 bit integer from 0 to 255 (_Unsigned _Byte) ~%% format
        PSHS    B             ; Save B on the stack
; *** Doing CompareOp                      
...
@True   LEAS    1,S           
        ...
        LDA     ,S+           ; get result off stack, fix stack
        LBNE    _IFDone_03    ; IF Not Equal then jump
; Expression is parsed                      
_IFTrue_03                      
; BY =67
        LDD     #$0043        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_BY       ; Save Numeric variable
; VY =-VY 
...
; FRAMECOUNT =FRAMECOUNT +1
        LDD     _Var_FRAMECOUNT   ; Get the value of the variable
        PSHS    D             ; Save the value on the stack
        LDD     #$0001        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
; Doing ADD:                      
...
; Starting if                      
        LDD     _Var_FRAMECOUNT   ; Get the value of the variable
        PSHS    D             ; Save the value on the stack
        LDB     #6            ; B is an 8 bit integer from 0 to 255 (_Unsigned _Byte) ~%% format
        PSHS    B             ; Save B on the stack
; *** Doing CompareOp                      
; Doing Function LessOrEqual                      
; LeftType= 5                      
...
@True   LEAS    1,S           
        ...
        LDA     ,S+           ; get result off stack, fix stack
        LBNE    _IFDone_04    ; IF Not Equal then jump
; Expression is parsed                      
_IFTrue_04                      
; GOSUB 9000
        JSR     _L9000        ; GOSUB 9000
; END IF 
_IFDone_04                      ; Label: END IF
; GOTO MainLoop
        JMP     _LMainLoop    ; GOTO MainLoop
_L9000
; REM  -- recolour the dome in place -- 
; FRAMECOUNT =0
        LDD     #$0000        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_FRAMECOUNT   ; Save Numeric variable
; COLORIDX =COLORIDX +1
        LDD     _Var_COLORIDX ; Get the value of the variable
        PSHS    D             ; Save the value on the stack
        LDD     #$0001        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
; Doing ADD:                      
...
@True   LEAS    1,S           
        ...
        LDA     ,S+           ; get result off stack, fix stack
        LBNE    _IFDone_05    ; IF Not Equal then jump
; Expression is parsed                      
_IFTrue_05                      
; COLORIDX =0
        LDD     #$0000        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
...
; Starting if                      
        ...
        PSHS    B             ; Save B on the stack
; *** Doing CompareOp                      
; Doing Function NotEqual                      
        LDX     #$FFFF        ; Default is True
        LDB     ,S+           ; Get V2 8 bit value, move the stack
        CLRA                  ; Zero extend unsigned 8-bit into D
        SUBD    ,S            ; Subtract V1 16 bit value
        BNE     >             ; Skip if NotEqual
        LDX     #$0000        ; Set as False
!       STX     ,S+           ; Save X and move the stack forward, we only need to save the result as an 8 bit value
        LDA     ,S+           ; get result off stack, fix stack
        LBNE    _IFDone_06    ; IF Not Equal then jump
; Expression is parsed                      
_IFTrue_06                      
; DOMENEXT =143
        LDD     #$008F        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_DOMENEXT ; Save Numeric variable
; END IF 
...
!       STX     ,S+           ; Save X and move the stack forward, we only need to save the result as an 8 bit value
        LDA     ,S+           ; get result off stack, fix stack
        LBNE    _IFDone_07    ; IF Not Equal then jump
; Expression is parsed                      
_IFTrue_07                      
; DOMENEXT =159
        LDD     #$009F        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_DOMENEXT ; Save Numeric variable
; END IF 
...
!       STX     ,S+           ; Save X and move the stack forward, we only need to save the result as an 8 bit value
        LDA     ,S+           ; get result off stack, fix stack
        LBNE    _IFDone_08    ; IF Not Equal then jump
; Expression is parsed                      
_IFTrue_08                      
; DOMENEXT =175
        LDD     #$00AF        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_DOMENEXT ; Save Numeric variable
; END IF 
...
; Starting if                      
        LDD     _Var_COLORIDX ; Get the value of the variable
        PSHS    D             ; Save the value on the stack
        LDB     #3            ; B is an 8 bit integer from 0 to 255 (_Unsigned _Byte) ~%% format
        PSHS    B             ; Save B on the stack
; *** Doing CompareOp                      
; Doing Function NotEqual                      
        LDX     #$FFFF        ; Default is True
...
!       STX     ,S+           ; Save X and move the stack forward, we only need to save the result as an 8 bit value
        LDA     ,S+           ; get result off stack, fix stack
        LBNE    _IFDone_09    ; IF Not Equal then jump
; Expression is parsed                      
_IFTrue_09                      
; DOMENEXT =191
        LDD     #$00BF        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
...
; Starting if                      
        LDD     _Var_COLORIDX ; Get the value of the variable
        PSHS    D             ; Save the value on the stack
        LDB     #4            ; B is an 8 bit integer from 0 to 255 (_Unsigned _Byte) ~%% format
        PSHS    B             ; Save B on the stack
; *** Doing CompareOp                      
; Doing Function NotEqual                      
        LDX     #$FFFF        ; Default is True
...
!       STX     ,S+           ; Save X and move the stack forward, we only need to save the result as an 8 bit value
        LDA     ,S+           ; get result off stack, fix stack
        LBNE    _IFDone_10    ; IF Not Equal then jump
; Expression is parsed                      
_IFTrue_10                      
; DOMENEXT =223
        LDD     #$00DF        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_DOMENEXT ; Save Numeric variable
; END IF 
...
; Starting if                      
        LDD     _Var_COLORIDX ; Get the value of the variable
        PSHS    D             ; Save the value on the stack
        LDB     #5            ; B is an 8 bit integer from 0 to 255 (_Unsigned _Byte) ~%% format
        PSHS    B             ; Save B on the stack
; *** Doing CompareOp                      
; Doing Function NotEqual                      
        LDX     #$FFFF        ; Default is True
...
!       STX     ,S+           ; Save X and move the stack forward, we only need to save the result as an 8 bit value
        LDA     ,S+           ; get result off stack, fix stack
        LBNE    _IFDone_11    ; IF Not Equal then jump
; Expression is parsed                      
_IFTrue_11                      
; DOMENEXT =239
        LDD     #$00EF        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_DOMENEXT ; Save Numeric variable
; END IF 
...
!       STX     ,S+           ; Save X and move the stack forward, we only need to save the result as an 8 bit value
        LDA     ,S+           ; get result off stack, fix stack
        LBNE    _IFDone_12    ; IF Not Equal then jump
; Expression is parsed                      
_IFTrue_12                      
; DOMENEXT =255
        LDD     #$00FF        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_DOMENEXT ; Save Numeric variable
; END IF 
...
; DOMECUR =DOMENEXT 
        LDD     _Var_DOMENEXT ; Get the value of the variable
;        PSHS    D             ; Save the value on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_DOMECUR  ; Save Numeric variable
; RETURN 
