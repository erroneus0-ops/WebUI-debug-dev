...
_LMainLoop
; SPRITE LOCATE 0,8,BY 
        LDB     #0            ; B is an 8 bit integer from 0 to 255 (_Unsigned _Byte) ~%% format
;        PSHS    B             ; Save B on the stack
;        PULS    B             ; Get value in B and Fix the stack
        PSHS    B             ; Save the sprite #
        LDB     #8            ; B is an 8 bit integer from 0 to 255 (_Unsigned _Byte) ~%% format
        PSHS    B             ; Save B on the stack
        CLRA                  ; Clear MSB of D
        PULS    B             ; Get value in B and Fix the stack
...
; Starting if                      
        LDD     _Var_BY       ; Get the value of the variable
        PSHS    D             ; Save the value on the stack
        LDB     #55           ; B is an 8 bit integer from 0 to 255 (_Unsigned _Byte) ~%% format
        PSHS    B             ; Save B on the stack
; *** Doing CompareOp                      
...
_IFTrue_03                      
; BY =55
        LDD     #$0037        ; D is a 16 bit integer from -32768 to 32767 (signed Integer) % format
;        PSHS    D             ; Save D on the stack
;        PULS    D             ; Get value off the stack, fix the stack
        STD     _Var_BY       ; Save Numeric variable
; VY =-VY 
