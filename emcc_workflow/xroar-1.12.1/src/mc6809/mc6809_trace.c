...
// Added for the WASM debugger UI (not part of upstream XRoar as of
// 1.12.1): returns just the number of bytes the instruction at `bytes`
// occupies -- page prefix, opcode, postbyte, and operand, all included.
// Reuses the exact same opcode table (instructions[3][256]) and the
// exact same addressing-mode dispatch as mc6809_trace_instruction()
// above, kept in this same file specifically so both share one source
// of truth for instruction layout rather than a second copy that could
// drift out of sync with it.
//
// Doesn't reuse next_byte()/struct byte_iter directly -- their nbytes
// field grows on every read rather than acting as a fixed bound, which
// wasn't obviously intentional from reading it, and a length-only
// function just needs a plain bounds-checked counter, so writing a
// small, directly-verifiable version was preferable to importing
// behavior that wasn't fully understood.
//
// `bytes` must point to at least MC6809_MAX_TRACE_BYTES (8) valid
// bytes -- the same buffer size XRoar's own trace_bytes field already
// uses, i.e. this isn't a new number being guessed at, it's reusing the
// one Ciaran already settled on as safe for any instruction on this
// CPU family. Callers reading from live memory near the top of
// addressable space should handle wraparound/clamping themselves.
...
unsigned mc6809_instruction_length(uint8_t *bytes) {
	...
	case INDEXED: {
		unsigned postbyte = bytes[index++];
		if ((postbyte & 0x80) != 0) {
			// 5-bit offset postbytes ((postbyte & 0x80) == 0)
			// need nothing further; everything else depends on
			// the low nibble, same as mc6809_trace_instruction's
			// INDEXED case above.
			unsigned idx_mode = postbyte & 0x0f;
			switch (idx_mode) {
			case IDX_OFF8:
			...
			case IDX_EXT16:
				...
				break;
			default:
				// PI1/PI2/PD1/PD2/OFF0/OFFB/OFFA/OFFA_7/OFFD/ILL_A:
				// no extra bytes beyond the postbyte itself.
				break;
			}
		}
	} break;
