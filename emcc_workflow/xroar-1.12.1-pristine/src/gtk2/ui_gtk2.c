...
static void *ui_gtk2_new(void *cfg) {
	...
	*uigtk2 = (struct ui_gtk2_interface){0};
...
#ifdef USE_SIGNAL_HANDLER
	g_unix_signal_add(SIGHUP, (GSourceFunc)handle_os_signal, uigtk2);
	g_unix_signal_add(SIGINT, (GSourceFunc)handle_os_signal, uigtk2);
#endif
	...
	ui_messenger_join_group(uigtk2->msgr_client_id, ui_tag_monochrome, MESSENGER_NOTIFY_DELEGATE(uigtk2_ui_state_notify, uigtk2));
...
static void uigtk2_ui_state_notify(void *sptr, int tag, void *smsg) {
	...
	case ui_tag_monochrome:
		uigtk2_toggle_action_set_active(uigtk2, "/MainMenu/ViewMenu/Monochrome", value ? TRUE : FALSE);
		break;
